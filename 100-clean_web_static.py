...
def do_clean(number=0):
    ...
    listRemoveFiles = local("ls -t ./versions | tail -{}"
                            .format(numberOfVersions - 1 
                                    if number <= 1 else numberOfVersions - number)).stdout
    local("rm -f {}".format(listRemoveFiles))

    with cd("/data/web_static/releases/"):
        numberOfReleases = int(run("ls -t /data/web_static/releases/ | wc -l").stdout)
        listRemoveReleases = local("ls -t | tail -{}"
                                   .format(numberOfReleases - 1 
                                           if number <= 1 else numberOfReleases - number)).stdout
        local("rm -f {}".format(listRemoveReleases))
